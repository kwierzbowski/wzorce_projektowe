...
@Component("advancedValidator_Reservation")
public class ReservationValidator_Advanced implements ReservationValidator {

    private final IRoomService_Query roomServiceQuery;
//    private final IRoomService roomService;

    @Autowired
    public ReservationValidator_Advanced(IRoomService_Query roomServiceQuery) {
        this.roomServiceQuery = roomServiceQuery;
    }

    @Override
    public boolean validate(MakeReservationDTO reservation) {
        if (!reservation.getStartTime().before(reservation.getEndTime()) || reservation.getEstimatedPrice() > 0) {
            return false;
        }
